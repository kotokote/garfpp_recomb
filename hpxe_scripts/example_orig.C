...
int main(int argc, char * argv[]) {
  ...
  double fieldlist[11] = {5.,2.,1.,0.5,0.2,0.1}; 
  // double fieldlist[5] = {2.,1.,0.5,0.2,0.1}; 
  for (int nef = 0; nef < 6; nef++) {
    double ef = fieldlist[nef];

    std::cout << "Electric Field of " << ef << " volt per cm" << "\n";
  //simulation parameters
  int npe = 10;                                  //number of primary electrons (avalanches) to simulate
  ...
  gas->SetPressure(p);
  gas->SetComposition("ne", 0.0, "xe",1000.0);              //since the implemented decay model is only valid for pure noble gases,
  ...
  double e0list[5] = {0.1,0.5,1.0,5.0,10.};
  // double e0list[5] = {0.00001};
  for (int ne0=0; ne0<5; ne0++){
    double e0 = e0list[ne0];
  ...
  for (int i = 0; i<npe;i++) {
    ...
    //intial conditions
    const double x0 = 0.0;
    const double y0 = 0.2;                                              // the primary electrons are released 0.2 cm far way from the bottom electrode and drift toward the top electrode
    const double z0 = 0.0;
    const double t0 = 0.;                          
