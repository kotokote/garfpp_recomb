...
// in version 20: changed default number of movie frames to 1, so that frame 0 prints out beginning info including beginning location and 
...
// 11. specify numberofmovieframes in movie - number of movie frames to use; 0 if want this output supressed - 1 by default
...
double eleng = 0; // default of 0 results in random energy between 0 and 7 eV along fcn
...
cout << "Electric Field of " << efield << " V/cm" << "\n";
...
cout << "Running " << numclouds << " instance(s) of this simulation, " << elpercloud << " electron-ion pair(s) each." << endl;

// convert fourth arg to runtime in ns
if (argc > 4 && atof(argv[4]) > 0) runtime = atof(argv[4]);
cout << "Runtime of " << runtime << " ns." << endl;
 
// if are five or more arguments, the 5th one is the energy of the electrons
if (argc > 5 && atof(argv[5]) >= 0) eleng = atof(argv[5]);
cout << "Initial electron energy input is " << eleng << " , where 0 results in an energy between 0 and 7 eV along 1/(E^2+7.6eV^2), and >=0 gives that value as a uniform initial energy in eV." << endl;
...
if (elpercloud > 1) cout << "Angle of ion column: " << eltheta*180/Pi << " degrees." << endl;

// 8th argument is choice of recombination condition
bool deBroglieRecomb = 1;
cout << "Recombination condition of: ";
if (argc > 8 && atoi(argv[8])==0) {
  deBroglieRecomb = 0;
  cout << deBroglieRecomb << " = Using Onsager radius as recombination condition.\n";
} else cout << deBroglieRecomb << " = Using the smaller of deBroglie wavelength or Onsager radius as recombination condition.\n";
...
cout << "Using input file for ion positions: " << use_input_file;
if (argc > 10 && use_input_file) cout << " - " << argv[10] << endl;
else cout << " = no" << endl;

// 11th argument: number of movie frames - pass to AvalancheCloud; default is on and =1 - on if > 0
int numberofmovieframes = 1;
...
cout << "Movie frames being printed: " << numberofmovieframes << " frame(s)" << endl;
...
//        else e0[jj] = RndmUniform()*8.0 ; // uniform energy distribution from 0 to 8 eV
      ...
      for (int ie=0;ie<ep[i].ne;ie++){
        double x0, y0, z0, t0, e0, potential0, mdi0;
        double x1, y1, z1, t1, e1, potential1, mdi, mdimax;
        double xion, yion, zion;
        int status, id;
        aval->GetElectronEndpoint(ie,x0,y0,z0,t0,e0,x1,y1,z1,t1,e1,status,id,potential0,potential1,xion,yion,zion,mdi0,mdi,mdimax);
	  ...
	  std::cout << "Electron " << id << " of " << ep[i].ne << " left the drift medium: (x1,y1,z1)= ( " << x1 << " , " << y1 << " , " << z1 
          ...
          std::cout << "(x0,y0-.2,z0)= ( " << x0 << " , " << y0-.2 << " , " << z0 << " ), t0= " << t0 << " , e0= " << e0 << " , potential0= " << potential0 << " , minDistIon0= " << mdi0 << "\n";
          std::cout << "(x1,y1-.2,z1)= ( " << x1 << " , " << y1-.2 << " , " << z1 << " ), t1= " << t1 << " , e1= " << e1 << " , potential1= " << potential1 << "\n";
          std::cout << "status = " << status << "\n";
          std::cout << "(xionf,yionf-.2,zionf)= ( " << xion << " , " << yion-.2 << " , " << zion << " ), minDistIon= " << mdi << " , minDistIonMax= " << mdimax << std::endl << std::endl;
        }
      }
      int nElastic;
      int nIonising;
