...
class AvalancheMicroscopic {
  ...
  public:
   ...
   void DisableInducedChargeCalculation() {useInducedCharge = false;}
    ...
    void GetElectronEndpoint(const int i,
                double& x0, double& y0, double& z0, double& t0, double& e0,
                double& x1, double& y1, double& z1, double& t1, double& e1,
                int& status, int& id, double& potential0, double& potential1,
                double& xi, double& yi, double& zi, double& mdi0, double& mdi, double& mdimax) const;
    void GetElectronEndpoint(const int i, 
                double& x0, double& y0, double& z0, double& t0, double& e0,
                double& x1, double& y1, double& z1, double& t1, double& e1,
    ...
    // Calculate an avalanche initiated by a set of ionizations with given
    // coordinates, energy and direction (random if not specified)
  bool AvalancheCloud(const int nIonization, const double x0[], const double y0[], const double z0[],
                           const double t0[], const double e0[],
          const double dx0[], const double dy0[], const double dz0[], bool deBroglieRecomb, const double movieframetime[], const int numberofmovieframes);
  ...
  private:
    ...
    struct electron {
      ...
      // Starting point
      double x0, y0, z0, t0;
      // Initial energy and potential energy
      // megan: added initial potential energy
      double e0, potential0;
      // Band
      int band;
      // Current position
      double x, y, z, t;    
      // Position of parent ion
      double xi, yi, zi;    
      // Current direction/wavevector
      double kx, ky, kz;
      // Current kinetic energy and potential energy
      // megan: added current potential energy
      double energy, potential;
      // Drift line
      std::vector<point> driftLine;
      double xLast, yLast, zLast;
      // megan: added Electron ID (number in cloud of size n, from 1 to n)
      int id;
      // megan: added mdi to store minDistIon and mdimax to store max distance from any ion
      double mdi0, mdi, mdimax;
    };
    ...
    // Cloud of ionizations transport
    bool TransportCloud(
	const int nIonization, 
	const double x0[], const double y0[], const double z0[], 
        const double t0[], const double e0[],
        const double dx0[], const double dy0[], const double dz0[], 
