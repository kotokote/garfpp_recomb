...
bool MediumMagboltz::GetElectronCollision(const double e, int& type, int& level, 
  ...
  // Azriel Modify from here on to add target atoms with thermal motion
  // split up the two cases of elastic (loss=0) and inelastic
  // treat the lossy case without thermal motion (at least initially)

  if (loss == 0.) {
    // std::cout << "Azriel DEBUG dx: " << dx << " dy: " << dy << "dz: " << dz << " dmod: " << sqrt(dx*dx + dy*dy + dz*dz) << " e: " << e << "\n";

    // choose a Maxwell Boltzman velocity for the target atom in the lab frame

    // mass in grams of the target molecule/atom
    const double mass_t = ElectronMassGramme / (rgas[igas]  - 1.);
    const double BoltzmannConstant_cgs = BoltzmannConstant * ElementaryCharge * 1e-8 ;
    const double boltz_sigma = sqrt(BoltzmannConstant_cgs*temperature/mass_t);
    
    // std::cout << "Azriel DEBUG temperature K: " << temperature << " Boltzman Constant in CGS: " << BoltzmannConstant_cgs << "mass_t: " << mass_t << " boltz_sigma: " << boltz_sigma << "\n";

    // obtain three components of the target molecule/atom velocity in the lab in cm/s
    const double target_vx = RndmGaussian(0., boltz_sigma);
    const double target_vy = RndmGaussian(0., boltz_sigma);
    const double target_vz = RndmGaussian(0., boltz_sigma);

    // get the unit vector in the target direction
    const double t_mod = sqrt( target_vx*target_vx+target_vy*target_vy+target_vz*target_vz);
    const double t_dx = target_vx/t_mod;
    const double t_dy = target_vy/t_mod;
    const double t_dz = target_vz/t_mod;

    // std::cout << "Azriel DEBUG target vx: " << target_vx << " vy: " << target_vy << "vz: " << target_vz << " v: " << sqrt(target_vx*target_vx+target_vy*target_vy+target_vz*target_vz) << "\n";

    // now rotate to the coordinate system where the electron (projectile) 
    // is traveling in the +x direction and the target atom in the x +y semi-plane
    // the +y removes the ambiguity of sin(theta_i) below
    // this rotation will be undone as the last step

    // Obtain here the rotation matrix that achieved the above (to be inverted later)

    // first obtain the vector k around which to rotate to align the electron momentum with +x
    // compute the cross product between the electron momentum direction and the x axis and normalize 
    // the vector to a unit vector
    double kx, ky, kz, kmod;
    kx = 0.; ky = dz; kz = -dy; kmod = sqrt( ky*ky + kz*kz);
    ky = ky/kmod; kz = kz/kmod;

    // now obtain the cosine of the angle between the electron momentum direction and the x axis
    double cos_pe_x = dx;
    // sin is always positive because opening angle is always 0 to pi 
    double sin_pe_x = sqrt( 1.0 - cos_pe_x * cos_pe_x );

    // now obtain the rotation matrix that turns electron momentum towards +x axis
    // from wiki roration along a u vector (in this case k)
    double r1xx = cos_pe_x+kx*kx*(1.-cos_pe_x);
    double r1xy = kx*ky*(1.-cos_pe_x)-kz*sin_pe_x;
    double r1xz = kx*kz*(1.-cos_pe_x)+ky*sin_pe_x;
    double r1yx = ky*kx*(1.-cos_pe_x)+kz*sin_pe_x;
    double r1yy = cos_pe_x+ky*ky*(1.-cos_pe_x);
    double r1yz = ky*kz*(1.-cos_pe_x)-kx*sin_pe_x;
    double r1zx = kz*kx*(1.-cos_pe_x)-ky*sin_pe_x;
    double r1zy = kz*ky*(1.-cos_pe_x)+kx*sin_pe_x;
    double r1zz = cos_pe_x+kz*kz*(1.-cos_pe_x);

    // obtain the second rotation matrix along x which puts the target momentum vector in the x +y plane 
    // first obtain the target momentum direction vector after the first (above) rotation
    double t_r1_x = r1xx * t_dx + r1xy * t_dy + r1xz * t_dz;
    double t_r1_y = r1yx * t_dx + r1yy * t_dy + r1yz * t_dz;
    double t_r1_z = r1zx * t_dx + r1zy * t_dy + r1zz * t_dz;

    // now compute the cosine of opening angle between the yz projection of the above vector and the +y axis
    // from dot product formula
    // the minus in the sin alpha formula is confusing but I checked the right handedness  
    double cos_alpha  = t_r1_y / sqrt( t_r1_y * t_r1_y + t_r1_z * t_r1_z);
    double sin_alpha = -t_r1_z / sqrt( t_r1_y * t_r1_y + t_r1_z * t_r1_z);
    
    // now get the second rotation which is around x
    double r2xx = 1.0;
    double r2xy = 0.; double r2xz = 0.; double r2yx=0.; double r2zx = 0.;
    double r2yy = cos_alpha;
    double r2yz = -sin_alpha;
    double r2zy = sin_alpha;
    double r2zz = cos_alpha;

    // overall rotation is the product of the two rotation matrices r2 * r1
    double r21xx = r2xx*r1xx+r2xy*r1yx+r2xz*r1zx;
    double r21xy = r2xx*r1xy+r2xy*r1yy+r2xz*r1zy;
    double r21xz = r2xx*r1xz+r2xy*r1yz+r2xz*r1zz;

    double r21yx = r2yx*r1xx+r2yy*r1yx+r2yz*r1zx;
    double r21yy = r2yx*r1xy+r2yy*r1yy+r2yz*r1zy;
    double r21yz = r2yx*r1xz+r2yy*r1yz+r2yz*r1zz;

    double r21zx = r2zx*r1xx+r2zy*r1yx+r2zz*r1zx;
    double r21zy = r2zx*r1xy+r2zy*r1yy+r2zz*r1zy;
    double r21zz = r2zx*r1xz+r2zy*r1yz+r2zz*r1zz;

    // check at this point that initial vectors get rotated as desired by the r2 * r1 matrix
    //double wx = r21xx*t_dx+r21xy*t_dy+r21xz*t_dz;
    //double wy = r21yx*t_dx+r21yy*t_dy+r21yz*t_dz;
    //double wz = r21zx*t_dx+r21zy*t_dy+r21zz*t_dz;

    //std::cout << "test of rotations : " << wx << " " << wy << " " << wz << "\n";

    // obtain quantities to do center of mass calculations
    const double fre = ElectronMassGramme / (ElectronMassGramme + mass_t);
    const double frt = mass_t / (ElectronMassGramme + mass_t);

    // obtain momentum of electron projectile in the lab frame
    // in eV/c units and its x y z components
    const double pmag_e = sqrt(2.0 * ElectronMassGramme * SpeedOfLight *SpeedOfLight 
			       * 1e18 * 1e-7 /( ElementaryCharge * 1e-15 )* e); 
    const double p_e_x = pmag_e * dx;
    const double p_e_y = pmag_e * dy;
    const double p_e_z = pmag_e * dz;


    // obtain momentum of target molecule/atom in lab frame in eV/c units
    // and its x y z components
    const double pmag_t = sqrt(target_vx*target_vx+
			       target_vy*target_vy+
			       target_vz*target_vz)/(SpeedOfLight*1e9)*
      mass_t*SpeedOfLight *SpeedOfLight * 1e18 * 1e-7 /( ElementaryCharge * 1e-15 );
    const double p_t_x = target_vx/(SpeedOfLight*1e9)*
      mass_t*SpeedOfLight *SpeedOfLight * 1e18 * 1e-7 /( ElementaryCharge * 1e-15 );
    const double p_t_y = target_vy/(SpeedOfLight*1e9)*
      mass_t*SpeedOfLight *SpeedOfLight * 1e18 * 1e-7 /( ElementaryCharge * 1e-15 );
    const double p_t_z = target_vz/(SpeedOfLight*1e9)*
      mass_t*SpeedOfLight *SpeedOfLight * 1e18 * 1e-7 /( ElementaryCharge * 1e-15 );


    // obtain cosine of opening angle between the electron projectile and the target 
    // molecule/atom in the lab frame (same a rotated)
    const double cos_theta_i = (p_e_x * p_t_x + p_e_y * p_t_y + p_e_z * p_t_z) / 
      (pmag_t * pmag_e); 

    // std::cout << " Azriel DEBUG for electron with energy (eV): " << e << " momentum in eV/c is: " << pmag_e << " and target atom momentum in eV/c is: " << pmag_t << " cosine angle between electron and target: " << cos_theta_i << "\n";

    // obtain components of momentum sum in lab for moving to CM frame
    // y component by definition positive (see above)
    const double prel_x = pmag_e + pmag_t * cos_theta_i;
    const double prel_y = pmag_t * sqrt(1.0 - cos_theta_i * cos_theta_i);

    // obtain CM frame momenta components and modulus
    const double p_e_x_cm = pmag_e - fre * prel_x;
    const double p_e_y_cm = -fre * prel_y;
    const double pmag_e_cm = sqrt( p_e_x_cm * p_e_x_cm + p_e_y_cm * p_e_y_cm);

    // std::cout << " electron p(x,y,z): " << p_e_x << " " <<  p_e_y << " " << p_e_z << "\n";    
    // std::cout << " target p(x,y,z): " << p_t_x << " " <<  p_t_y << " " << p_t_z << "\n";
    // std::cout << " electron cm p(x,y): " << p_e_x_cm << " " << p_e_y_cm << "\n";
    
    // now produce the scatter in the CM

    // get cos and sin of the incoming momenta in CM
    double theta_e_cm = atan(p_e_y_cm / p_e_x_cm); 
    // megan: following statement has no effect - was supposed to assign this to something?
    if ( p_e_x_cm < 0. ) { theta_e_cm + TwoPi/2.0; }
    
    // choose a random scattering angle direction (2D)
    // for test can put this angle to zero and we should obtain he initial directions after 
    double theta_cm = TwoPi * RndmUniform();
    
    const double p_e_prime_x_cm = pmag_e_cm * cos(theta_e_cm+theta_cm);
    const double p_e_prime_y_cm = pmag_e_cm * sin(theta_e_cm+theta_cm);
    const double p_t_prime_x_cm = -p_e_prime_x_cm;
    const double p_t_prime_y_cm = -p_e_prime_y_cm;

    // revert back to the lab coordinates (rotated)
    const double p_e_prime_x = p_e_prime_x_cm + fre * prel_x;
    const double p_e_prime_y = p_e_prime_y_cm + fre * prel_y;
    const double p_t_prime_x = p_t_prime_x_cm + frt * prel_x;
    const double p_t_prime_y = p_t_prime_y_cm + frt * prel_y;

    // undo the rotations (already in the lab)
    // multiply by inverse of r2 * r1 (simply the transpose since it is a rotation matrix)
    const double p_e_final_x = r21xx*p_e_prime_x+r21yx*p_e_prime_y;
    const double p_e_final_y = r21xy*p_e_prime_x+r21yy*p_e_prime_y;
    const double p_e_final_z = r21xz*p_e_prime_x+r21yz*p_e_prime_y;

    const double p_t_final_x = r21xx*p_t_prime_x+r21yx*p_t_prime_y;
    const double p_t_final_y = r21xy*p_t_prime_x+r21yy*p_t_prime_y;
    const double p_t_final_z = r21xz*p_t_prime_x+r21yz*p_t_prime_y;

    const double p_e_final_mag = sqrt(p_e_final_x*p_e_final_x + p_e_final_y*p_e_final_y + p_e_final_z*p_e_final_z); 
    
    // compare with naive approximation that adds the velocity of target in lab to the electron velocity 
    
    dz = p_e_final_z/p_e_final_mag; dy = p_e_final_y/p_e_final_mag; dx = p_e_final_x/p_e_final_mag;
    e1 = p_e_final_mag*p_e_final_mag/(2.0 * ElectronMassGramme * SpeedOfLight *SpeedOfLight * 1e18 * 1e-7 /( ElementaryCharge * 1e-15 ));
    // std::cout << "Azriel DEBUG angle after collision " << dx << " " << dy << " " << dz << "\n";
    // std::cout << "Azriel DEBUG electron energy after collision " << e1 << "\n";
    
  } else { 
    // lossy case when electron target system lost energy in collision
    ...
    // std::cout << "Azriel DEBUG igas: " << igas << " rgas[igas]: " << rgas[igas] << "gas[igas]: " << gas[igas] << " s1: << s1 << " s2: " << s2 << " e: " << e << "\n"; 
    
    // Update the energy. 
    e1 = std::max(e * (1. - loss / (s1 * e) - 2. * d / s2), Small);
    // std::cout << "Azriel DEBUG e: " << e << " e1: " << e1 << "loss: " << loss << "\n"; 
...
// Azriel added Onsager radius function
double MediumMagboltz::GetOnsagerRadius() {
  // For now this function is only delivering the Onsager radius assuming is 
  // only Xenon in the mix
  // need to weigh the various components

  // Onsager radius 
  double or_rx, or_ex, or_rad;
  // density
  or_rx =  Medium::GetPressure()/(62363.32* Medium::GetTemperature());
  // epsilon relative dielectric
  or_ex = (1+(20.18*or_rx))/(1-(10.09*or_rx)); 
  or_ex = or_ex*VacuumPermittivity;

  SetDielectricConstant(or_ex);

  // onsager radius
  // radius for which potential energy beywwen two electron charges equals the thermal energy
  or_rad = ElementaryCharge/(4*Pi*or_ex*BoltzmannConstant*Medium::GetTemperature()); 

  rOnsager = or_rad;
//    std::cout << "Onsager Radius :" << rOnsager << "\n";
  return rOnsager;
