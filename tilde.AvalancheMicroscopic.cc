...
AvalancheMicroscopic::AvalancheCloud(
  ...
  return TransportCloud(nIonization, x0, y0, z0, t0, e0, dx0, dy0, dz0, true, false);
...
bool 
AvalancheMicroscopic::TransportCloud(
  ...
  double fLim = 0.;
  double finer_tracking_factor = 100.;
  double vx, vy, vz;
  double kx, ky, kz;
  electron newElectron;
  ...
  for (int ionization = 0; ionization < nIonization; ionization++) {
	...
	DielectricConst =  medium->GetDielectricConstant();
      ...
      // std::cout <<  OnsagerRadius << " Onsager Radius in cm \n";
	...
	newElectron.hole = false;
      ...
      // x0,y0,z0 positions represent inmobile parent ion position
      // place the corresponding electron at 1 Onsager radius away from the ion in the 
      // initial direction
      newElectron.x0 = x0[ionization];  newElectron.x = x0[ionization]+thedx0*OnsagerRadius;
      newElectron.y0 = y0[ionization];  newElectron.y = y0[ionization]+thedy0*OnsagerRadius;
      newElectron.z0 = z0[ionization];  newElectron.z = z0[ionization]+thedz0*OnsagerRadius;
  ...
  if (n1Size > 0) { 
	...
	if (iE != iE2) {
      ...
      stack[iE].energy = stack[iE].e0 - potential;

      std::cout << "Ek0 (ID,e0,potential,Ek) " << iE << " " << stack[iE].e0 << " " << potential << " " << stack[iE].energy << "\n";
  ...
  while (1) {
    ...
    for (int iE = nSize; iE--;) {
      ...
      // variable to save the time of the oldest other electron
      double toldest = 1e99;
      double tnewest = -1e99;
	...
	// distance to ion (calculations assume we are computing the force on a positive charge)
	...
	if (iE != iE2) {
      ...
      while (1) {
        ...
        while (1) {
          // Sample the flight time.
          r = RndmUniformPos();
	  // Azriel increase the null collision rte by 100 to make sure field transport and 
	...
	if (counter_steps % 1000000 ==0) {
	  std::cout << "step counter at " << counter_steps << " " << x << " " << y << " " << z << " " << t << " " << newEnergy << "\n";
	}
	...
	// if (iE == 80) {
	//  std::cout << "    At " << x << " " << y << " " << z << " " << t << " with E(eV)= " << newEnergy << " " << potential << "\n";    
	// }
	...
	if ( (newEnergy + potential < 0.) && (minDistIon < OnsagerRadius) ) {
	  // recombine
	  std::cout << "Electron will recombine, Ek= " << newEnergy << " and potential= "<< potential << "at R= " << minDistIon << "\n";
	  std::cout << "Time to recombine = " << t <<  "\n";
	  std::cout << "Origin x,y,z " << stack[iE].xi << " " <<  stack[iE].yi << " " <<  stack[iE].zi << "\n";
	  std::cout << "Final x,y,z " << x << " " <<  y << " " <<  z << "\n";
	  // for the absorbing ion, make that stack element dissapear storing the parent ion position
	  ...
	  // ion as well
	  double xtemp, ytemp, ztemp, ttemp, etemp;
	  int miE2 =  minDistIonIndex;

	  // swap ion positions between electrons
	  xtemp = stack[iE].x0; ytemp = stack[iE].y0; ztemp = stack[iE].z0;
	  stack[iE].x0 = stack[miE2].x0;  stack[iE].y0 = stack[miE2].y0; stack[iE].z0 = stack[miE2].z0;
	  stack[miE2].x0 = xtemp; stack[miE2].y0 = ytemp; stack[miE2].z0 = ztemp;
	  ...
	  // for instance how long or distance before recombination
          ...
          ok = false;
          if (debug) {
            std::cout << className << "::TransportElectronCloud:\n";
            std::cout << "    Electron/hole pair recombined.\n";
            std::cout << "    At " << x << ", " << y << "," << z << "\n";
          }
	  // this breaks out of the tracing the individual iE electron
          break;
        ...
        // Continue with the next electron/hole?
	// change codition to make sure it keeps tracking until it catches up in time with the oldest electron
